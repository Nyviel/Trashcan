...
function checkVotesComplete(io, room) {
	const state = room.gameState;
	// Non-host, non-fake players vote (or everyone except host)
	const voters = state.drawers;
	const allVoted = voters.every((p) => state.votes[p.id]);
	if (!allVoted) return;
...
function handleEvent(io, socket, room, event, args) {
	...
	switch (event) {
		...
		case "fake:stroke": {
			...
			// Store stroke in history and relay to all
			state.strokes.push(data);
			socket.to(room.code).emit("fake:stroke", data);
			// Drawer submitted their stroke — advance immediately
		...
		case "fake:vote": {
			...
			io.to(room.code).emit("fake:voteUpdate", {
				...
				totalVoters: state.drawers.length,
			});
